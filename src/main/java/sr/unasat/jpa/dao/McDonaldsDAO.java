...
public class McDonaldsDAO {
    ...
    public int updateMcDonalds(McDonalds mcDonalds) {
        entityManager.getTransaction().begin();
        String sql = "update McDonalds m set m.name = :address, m.phoneNumber = :phoneNumber, m.code = :code, m.city = :city where m.id = :id";
        Query query = entityManager.createQuery(sql);
        query.setParameter("id", mcDonalds.getId());
        query.setParameter("address", mcDonalds.getName());
